...
if os.path.exists(panns_repo_path) and panns_repo_path not in sys.path:
    ...
    print(
        f"Warning: {panns_repo_path} not found or already in sys.path. "
...
class PodFineTunedClassifier(nn.Module):
    def __init__(
    ...
    ):  # PANNs Cnn14 expects 16kHz
        super().__init__()

        # Define the device for Apple Silicon (MPS)
        if torch.backends.mps.is_available():
            self.device = torch.device("mps")
            print("Using Apple Silicon GPU (MPS) for training and inference.")
        else:
        ...
        panns_classes_num_audioset = (
            527  # Number of classes PANNs was trained on (AudioSet)
        )
        ...
        try:
            ...
            checkpoint = torch.load(
                panns_weights_path, map_location="cpu", weights_only=False
            )  # Load to CPU first
        ...
        if not freeze_panns:
            ...
            print(
                f"PANNs model layers partially unfrozen: {', '.join(layers_to_unfreeze)}"
            )
        else:
        ...
        panns_embedding_size = (
            self.panns_model.fc_audioset.in_features
        )  # This should be 2048 for Cnn14
...
def test_panns_raw_wav(
    audio_filepath,
    trained_classifier_path="./panns_classifier_head.pth",
    panns_base_weights_path="../audio-receiver/pretrained/Cnn14_16k_mAP=0.438.pth",
):

    # IMPORTANT: Replace with YOUR actual labels
    # This should match the order you used during training.
    label_to_idx = {"greetings": 0, "alarms": 1, "speech": 2, "music": 3}
    ...
    input_waveform_tensor = load_and_preprocess_raw_audio(
        audio_filepath, original_sr=48000, target_sr=16000
    )
    ...
    model.load_state_dict(
        torch.load(trained_classifier_path, map_location=model.device)
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Total samples: {len(train_dataset) + len(eval_dataset) + len(test_dataset)}"
    )
    print(
        f"Split ratio: {len(train_dataset):.1%}/{len(eval_dataset):.1%}/{len(test_dataset):.1%}"
    )

    # Model, loss, optimizer
    model = PodFineTunedClassifier(
        num_classes=num_classes,
        panns_weights_path="../audio-receiver/pretrained/Cnn14_16k_mAP=0.438.pth",
        freeze_panns=True,
        ...
        [
            {
                "params": [
                    ...
                    if "panns_model" in n and p.requires_grad
                ],
                "lr": pretrained_lr,
            },
            {
                "params": [
                    p for n, p in model.named_parameters() if "panns_model" not in n
                ],
                "lr": head_lr,
            },
        ]
    ...
    print(
        f"Learning rates: {pretrained_lr} for pre-trained layers, {head_lr} for classification head"
    )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(
            f"Epoch {epoch+1}/{NUM_EPOCHS} | Train Loss: {train_loss:.4f} | Train Acc: {train_acc:.4f} | Eval Loss: {eval_loss:.4f} | Eval Acc: {eval_acc:.4f}"
        )
        ...
        if best_eval_acc == 1.0:
            print(
                f"Early stopping triggered after {epoch+1} epochs. as 100% accuracy reached."
            )
            break
