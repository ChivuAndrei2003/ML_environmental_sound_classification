    ...
    def __init__(self, root_dir, target_sr=16000, split_type=None):
        ...
        if split_type is not None:
            ...
            if split_type == "train":
                ...
                print(
                    f"Created training split with {len(split_indices)} samples ({train_size/len(indices):.1%})"
                )
            elif split_type == "eval":
                split_indices = eval_indices
                print(
                    f"Created evaluation split with {len(split_indices)} samples ({eval_size/len(indices):.1%})"
                )
            elif split_type == "test":
                split_indices = test_indices
                print(
                    f"Created test split with {len(split_indices)} samples ({test_size/len(indices):.1%})"
                )
        ...
        print(
            f"Loaded {len(self.samples)} samples with {len(self.label_to_idx)} classes"
        )
...
if __name__ == "__main__":
    ...
    train_split_dataset = SplitAudioFolderDataset(
        data_dir, target_sr=16000, split_type="train"
    )
    eval_split_dataset = SplitAudioFolderDataset(
        data_dir, target_sr=16000, split_type="eval"
    )
    test_split_dataset = SplitAudioFolderDataset(
        data_dir, target_sr=16000, split_type="test"
    )
